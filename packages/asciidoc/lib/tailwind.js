function g(l, c) {
  return { handler: l, config: c };
}
g.withOptions = function (l, c = () => ({})) {
  function s(r) {
    return { handler: l(r), config: c(r) };
  }
...
var y = {
  ...
  R = m(({ addBase: l, addComponents: c, addUtilities: s }) => {
    let r = 'prose',
      n = `:not(:where([class~=".not-${r}"],[class~=".not-${r}"] *))`,
      e = `.${r}`,
      u = Object.fromEntries(
        ['slate', 'zinc', 'neutral', 'gray', 'stone'].reduce(
          (p, o) =>
            p.set(`${e}-${o}`, {
              '--faa-prose-color-50': a[o][50],
              '--faa-prose-color-100': a[o][100],
              '--faa-prose-color-200': a[o][200],
              '--faa-prose-color-300': a[o][300],
              '--faa-prose-color-400': a[o][400],
              '--faa-prose-color-500': a[o][500],
              '--faa-prose-color-600': a[o][600],
              '--faa-prose-color-700': a[o][700],
              '--faa-prose-color-800': a[o][800],
              '--faa-prose-color-900': a[o][900],
              '--faa-prose-color-invert-50': a[o][900],
              '--faa-prose-color-invert-100': a[o][800],
              '--faa-prose-color-invert-200': a[o][700],
              '--faa-prose-color-invert-300': a[o][600],
              '--faa-prose-color-invert-400': a[o][500],
              '--faa-prose-color-invert-500': a[o][400],
              '--faa-prose-color-invert-600': a[o][300],
              '--faa-prose-color-invert-700': a[o][200],
              '--faa-prose-color-invert-800': a[o][100],
              '--faa-prose-color-invert-900': a[o][50],
            }),
          new Map(),
        ),
      ),
      t = (p) => {
        let o = Object.entries(p).reduce((i, [f, h]) => {
          let k =
            /^(?<targetSelector>\.?[\w-]+(?:\s+\w+)+)(?<pseudoSelector>:{1,2}[\w\-()]+)$/.exec(
              f,
            )?.groups;
          if (k) {
            let { targetSelector: d, pseudoSelector: v } = k;
            if (d && v) return i.set(`${e} :where(${d})${v}${n} `, h);
          }
          return i.set(`${e} :where(${f})${n}`, h);
        }, new Map());
...
export { R as tailwindAsciidocTypography };
