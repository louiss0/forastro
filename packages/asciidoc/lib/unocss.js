var c = {
  ...
  f = {
    ...
    h1: { 'font-size': 'var(--faa-prose-step-5)' },
    h2: { 'font-size': 'var(--faa-prose-step-4)' },
    h3: { 'font-size': 'var(--faa-prose-step-3)' },
    h4: { 'font-size': 'var(--faa-prose-step-2)' },
    h5: { 'font-size': 'var(--faa-prose-step-1)' },
    abbr: {
      cursor: 'help',
    ...
    'p:has(.keyseq)': { display: 'flex', gap: 'var(--faa-prose-space-4)' },
    'h1 > small, h2 > small, h3 > small, h4 > small, h5 > small': {
      'font-size': '60%',
      padding: 'var(--faa-prose-space-2) var(--faa-prose-space-4)',
    },
    'code, kbd, pre, samp': { 'font-size': 'var(--faa-prose-step-neg-1)' },
    pre: {
      'white-space': 'pre-wrap',
      'line-height': '1.45',
      'text-rendering': 'optimizeSpeed',
    },
    small: { 'font-size': 'var(--faa-prose-step-neg-2)' },
    mark: {
  ...
  d = {
    ...
    '#footer': { 'max-width': 'none', padding: 'var(--faa-prose-space-8)' },
  ...
  g = {
    ...
    '.toc ul': { 'font-size': 'var(--faa-prose-step-neg-1)' },
    '.toc ul li:has(ul)': {
      display: 'flex',
      'flex-direction': 'column',
    ...
    '.admonitionblock.tip': { color: 'var(--faa-prose-tip-color)' },
    '.admonitionblock.note': { color: 'var(--faa-prose-note-color)' },
    '.admonitionblock.important': { color: 'var(--faa-prose-important-color)' },
    '.admonitionblock.warning': { color: 'var(--faa-prose-warning-color)' },
    '.admonitionblock.caution': { color: 'var(--faa-prose-caution-color)' },
    '.button': {
      'padding-inline': 'var(--faa-prose-space-2)',
    ...
    '.menuseq': { color: 'var(--faa-prose-color-700)' },
    '.menuseq .caret': { color: 'var(--faa-prose-color-400)' },
    '.menuseq .menuitem': { color: 'var(--faa-prose-color-500)' },
    '.sidebarblock': {
    ...
    '.quoteblock blockquote': { gap: 'var(--faa-prose-space-2)' },
    '.quoteblock .attribution': { 'font-size': 'var(--faa-prose-step-neg-1)' },
    '.dlist dl': {
      display: 'flex',
      'flex-direction': 'column',
      gap: 'var(--faa-prose-space-4)',
    },
    '.dlist dl dt': { 'font-weight': 'bold' },
    '.dlist dl dd': {
    ...
    'blockquote, quote p': { 'line-height': '1.6' },
    table: {
  ...
  v = {
    ...
    'pre.nobreak, :not(pre).nobreak': { 'word-wrap': 'normal' },
    'pre.nowrap, :not(pre).nowrap': { 'white-space': 'nowrap' },
    'pre.pre-wrap, :not(pre).pre-wrap': { 'white-space': 'pre-wrap' },
      ...
      rules: [
        ...
        [
          ...
          ([, r], { theme: i }) => {
            ...
            if (!(!a || typeof a == 'string'))
              return {
                '--faa-prose-color-50': a[50],
                '--faa-prose-color-100': a[100],
                '--faa-prose-color-200': a[200],
                '--faa-prose-color-300': a[300],
                '--faa-prose-color-400': a[400],
                '--faa-prose-color-500': a[500],
                '--faa-prose-color-600': a[600],
                '--faa-prose-color-700': a[700],
                '--faa-prose-color-800': a[800],
                '--faa-prose-color-900': a[900],
              };
          },
          { layer: 'typography' },
        ],
        [
          ...
          ([, r], { theme: i }) => {
            ...
            if (!(!a || typeof a == 'string'))
              return {
                '--faa-prose-color-invert-50': a[900],
                '--faa-prose-color-invert-100': a[800],
                '--faa-prose-color-invert-200': a[700],
                '--faa-prose-color-invert-300': a[600],
                '--faa-prose-color-invert-400': a[500],
                '--faa-prose-color-invert-500': a[400],
                '--faa-prose-color-invert-600': a[300],
                '--faa-prose-color-invert-700': a[200],
                '--faa-prose-color-invert-800': a[100],
                '--faa-prose-color-invert-900': a[50],
              };
          },
          { layer: 'typography' },
        ],
      ],
      preflights: [{ layer: 'typography', getCSS: b(e) }],
    };
  };
function b(e) {
  let p = `:not(:where(.not-${e},.not-${e} *))`,
    n = `.${e}`;
  return () =>
    Object.entries(m).map(([o, r]) => {
      let a = `{
${Object.entries(r).map(([s, t]) =>
  /^<\w+>$/.test(t) ? `	${s}:'${t}';` : `	${s}:${t};`,
).join(`
`)}
}
`;
      if (o.startsWith('@property')) return `${o} ${a}`;
      let l =
        /^(?<targetSelector>\.?[\w-]+(?:\s+\w+)+)(?<pseudoSelector>:{1,2}[\w\-()]+)$/.exec(
          o,
        )?.groups;
      if (l) {
        let { targetSelector: s, pseudoSelector: t } = l;
        if (s && t) return `${n} :where(${s})${t}${p} ${a}`;
      }
      return `${n} :where(${o})${p} ${a}`;
    }).join(`
`);
}
export { b as getCSSWithSelectorName, h as presetAsciidocTypography };
