...
describe('Testing internal tools', () => {
  test('AsciidocProcessorController instances are isolated', () => {
  ...
  test(`asciidocConfigObjectSchema returns an attributes.sourceHighlighter.
            When the object that is passed in is empty`, () => {
    const result = asciidocConfigObjectSchema.parse({});
  ...
  describe('Testing getAsciidocPaths', () => {
    ...
    it('gets paths from the specified directory only', async () => {
      ...
      expect(
        result.every((value) => !value.startsWith(specifiedDirectory)),
      ).toBeTruthy();
    });

    it('throws an error if the folder name that is passed in is empty', async () => {
  ...
  it('throws an error when a file is a not an m js or ts file', async () => {
    try {
      const result = await loadAsciidocConfig(`${MOCK_FOLDER}/false-config`);
      ...
      expect(newError.errors[0]?.message).toMatchInlineSnapshot(
  ...
  it("throws an error when the config file doesn't have the right properties", async () => {
    try {
      ...
      expect(
        newError.errors.every((error) =>
          error?.message.match(/Unrecognized key\(s\) in object: '\w+'/),
        ),
      ).toBeTruthy();
    }
  });
