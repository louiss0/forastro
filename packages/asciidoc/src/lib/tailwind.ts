...
export const tailwindAsciidocTypography: ReturnType<typeof plugin> = plugin(
  ({ addBase, addComponents, addUtilities }) => {
    ...
    const proseColorSelectorsObject = Object.fromEntries(
      ['slate', 'zinc', 'neutral', 'gray', 'stone'].reduce(
        (proseColorSelectorsMap, color) => {
          return proseColorSelectorsMap.set(
            `${typographySelectorClassName}-${color}`,
            {
              '--faa-prose-color-50': colors[color as keyof typeof colors][50],
              '--faa-prose-color-100':
                colors[color as keyof typeof colors][100],
              '--faa-prose-color-200':
                colors[color as keyof typeof colors][200],
              '--faa-prose-color-300':
                colors[color as keyof typeof colors][300],
              '--faa-prose-color-400':
              ...
              '--faa-prose-color-700':
                colors[color as keyof typeof colors][700],
              '--faa-prose-color-800':
                colors[color as keyof typeof colors][800],
              '--faa-prose-color-900':
                colors[color as keyof typeof colors][900],
              // For reverse colors
              '--faa-prose-color-invert-50':
                colors[color as keyof typeof colors][900],
              '--faa-prose-color-invert-100':
                colors[color as keyof typeof colors][800],
              '--faa-prose-color-invert-200':
                colors[color as keyof typeof colors][700],
              '--faa-prose-color-invert-300':
                colors[color as keyof typeof colors][600],
              '--faa-prose-color-invert-400':
                colors[color as keyof typeof colors][500],
              '--faa-prose-color-invert-500':
                colors[color as keyof typeof colors][400],
              '--faa-prose-color-invert-600':
                colors[color as keyof typeof colors][300],
              '--faa-prose-color-invert-700':
                colors[color as keyof typeof colors][200],
              '--faa-prose-color-invert-800':
                colors[color as keyof typeof colors][100],
              '--faa-prose-color-invert-900':
                colors[color as keyof typeof colors][50],
            },
          );
        },
      ...
      const mapWithProseSelectorKeys = Object.entries(object).reduce(
        (classMap, [classSelector, classProperties]) => {
          ...
          if (selectorWithPseudoSelectorGroups) {
            const { targetSelector, pseudoSelector } =
              selectorWithPseudoSelectorGroups;
    ...
    addComponents([
      ...
      generateClassObjectWithProperProseSelectorNamesAsKeys(
